...
// Deliberately `any`, not `unknown`: every caller below accesses response-shaped properties
// (checkRuns.check_runs, comments.some(...), prs.filter(...)) directly off the resolved value with no
// narrowing step, matching how this always actually ran as untyped JS -- typing this `unknown` would
// just force a wall of `as` casts at every call site without adding any real safety, since the GitHub
// API's actual response shape isn't validated here either way.
export type GithubApi = (path: string, options?: { method?: string; body?: string; headers?: Record<string, string> }) => Promise<any>;
 ...
 *  module-level token. */
function makeGithubApi(token: string): GithubApi {
...
export async function findStuckChecksForPr(
  pr: PullRequestRef,
  requiredContexts: Set<string>,
  { githubApi, owner, repoName, thresholdMinutes }: CommentScope & { thresholdMinutes: number },
): Promise<StuckCheck[]> {
...
export async function hasExistingWatchdogComment(prNumber: number, { githubApi, owner, repoName }: CommentScope): Promise<boolean> {
  const comments = await githubApi(`/repos/${owner}/${repoName}/issues/${prNumber}/comments?per_page=100`);
  return comments.some((comment: { body?: string }) => comment.body?.includes(MARKER));
}

async function postComment(prNumber: number, stuckChecks: StuckCheck[], { githubApi, owner, repoName }: CommentScope) {
  const lines = [
    MARKER,
...
export async function runStuckCheckWatchdog({
  ...
  const prs = (await githubApi(`/repos/${owner}/${repoName}/pulls?state=open&per_page=100`)).filter((pr: PullRequestRef) => !pr.draft);
  let flaggedCount = 0;
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  ...
  await runStuckCheckWatchdog({ githubApi: makeGithubApi(token), owner: owner!, repoName: repoName!, thresholdMinutes, dryRun });
}
